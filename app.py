...
st.markdown(
    ...
    <style>
    .stApp {
        background: url("https://images.unsplash.com/photo-1639322537228-f710d846310a?q=80&w=2500&auto=format&fit=crop");
...
# Gebruik 'gemini-pro' als 'gemini-1.5-flash' een 404 geeft
    ...
    with st.spinner("Analyseert..."):
        if "youtube.com/watch" in user_input or "youtu.be/" in user_input:
            try:
                ...
                st.error("Kon video niet samenvatten.")
        else:
            try:
                ...
                st.error(f"Fout bij AI (check je API-toegang in Google AI Studio): {e}")
